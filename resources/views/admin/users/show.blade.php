...
@section('content')
    ...
    <div class="card">
        ...
        <div class="card-body mb-2">
            @if ($user == null)
                <a href="{{ route('admin.users.pickup_report') }}?student&campus={{ request()->input('campus') ?? 'MC' }}&class={{ request()->input('class') ?? $selected_class->id }}&btn_pdf_front"
                    class="btn btn-primary float-left m-2">Print Pdf (Front Side)</a>
                <a href="{{ route('admin.users.pickup_report') }}?student&campus={{ request()->input('campus') ?? 'MC' }}&class={{ request()->input('class') ?? $selected_class->id }}&btn_pdf_back"
                    class="btn btn-primary float-left m-2">Print Pdf (Back Side)</a>
            @else
                <a href="{{ route('admin.users.show', $user) }}?print=pdf_front" class="btn btn-primary float-left m-2">Print
                    Pdf (Front Side)</a>
                <a href="{{ route('admin.users.show', $user) }}?print=pdf_back" class="btn btn-primary float-left m-2">Print
                    Pdf (Back Side)</a>
            @endif
                            ...
                            <div class="text-center float-right" style="width:11.2cm;padding-right:20px;padding-left:0px">
                                ...
                                <div class="p-1 mb-4 bg-white" style="font-size:14px;">
                                    <span class="float-left"><strong>Class:
                                            {{ str_replace('Morning', '', str_replace('Afternoon', '', $user->class->name)) }}</strong></span>&nbsp;
                                    <span class="float-right"><strong>School Year:
                                            {{ date('Y') - 1 . '-' . date('Y') }}</strong></span>
                                </div>
