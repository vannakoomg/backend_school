...
class NotificationApiController extends Controller
{
    ...
    public function markAsRead(){
        ...
        return response()->json(['status'=>true,'message'=>'All message of user '. auth()->user()->name . ' mark as read.','data'=>[]], $this->successStatus);
    
    }

    public function getNotificationList(Request $request){

        if(!$request->has('read') || !$request->has('firebasekey'))
            return response()->json(['status'=>false,'message'=>'Require read and firebasekey paramater for this APi.','data'=>[]], 401);

        if($request->input('read')==1){
           // $data = Auth::user()->notifications()->orderBy('created_at','desc')->paginate(10);
            $data = Auth::user()->notifications()->where(function ($q) use($request){    
                // $q->where('data','LIKE','%"fcmTokens":"' . $request->firebasekey  . '"%')->orWhere('data','LIKE','%"fcmTokens":[%"' . $request->firebasekey  . '"%]%');
                $q->Where('data','LIKE','%"fcmTokens":[%"' . $request->firebasekey  . '"%]%')->where('data','not like','%"appname":"Fingerprint Notification"%')->where('data','not like','%"appname":"ICS News"%')->where('data','not like','%"appname":"ICS Feedback"%')->where('data','not like','%"appname":"Order Notification"%')->where('data','not like','%"appname":"Top-up Notification"%')->where('data','not like','%"appname":"Assignment"%');
                //orWhere('data','LIKE','%"fcmTokens":[%"' . $request->firebasekey  . '"%]%')->where('data','not like','%"appname":"Fingerprint Notification"%')->where('data','not like','%{"title":"Assignment"%');
                
            })
            ->orderBy('created_at','desc')->paginate(10);
        }elseif($request->input('read')==2)
            $data = Auth::user()->unReadnotifications()->where(function ($q) use($request){    
                //$q->where('data','LIKE','%"fcmTokens":"' . $request->firebasekey  . '"%')->orWhere('data','LIKE','%"fcmTokens":[%"' . $request->firebasekey  . '"%]%')->where('data','not like','%"appname":"Fingerprint Notification"%');
                $q->Where('data','LIKE','%"fcmTokens":[%"' . $request->firebasekey  . '"%]%')->where('data','not like','%"appname":"Fingerprint Notification"%')->where('data','not like','%"appname":"ICS News"%')->where('data','not like','%"appname":"ICS Feedback"%')->where('data','not like','%"appname":"Order Notification"%')->where('data','not like','%"appname":"Top-up Notification"%')->where('data','not like','%"appname":"Assignment"%');
            })
            ->orderBy('created_at','desc')->paginate(10);
        else 
        ...
        return response()->json(['status'=>true,'message'=>'Notification List','data'=>$data], $this->successStatus);
    }

    public function marktNotificationRead(Request $request){

        if(!$request->has('trid'))
            return response()->json(['status'=>false,'message'=>'Notitication ID is Required for this API.','data'=>[]], 401);
        
            $data = Auth::user()->notifications()->find($request->trid);
            if($data) $data->markAsRead();

        return response()->json(['status'=>($data?true:false),'message'=>'Notification Detail','data'=>$data], $this->successStatus);
